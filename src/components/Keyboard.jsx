...
import { useUser } from '../util/contextapi'
import { keys } from './keys'
import Key from './Key'

const Keyboard = () => {
const { lights} = useUser();
  return (
    <div className='flex flex-col justify-start items-center bg-[#272729] rounded-[42px] w-5xl h-[702px]'>
        ...
        <div className='flex flex-row justify-between items-center mb-2 w-full h-[342px]'>
            ...
            <div className={`flex flex-wrap gap-1 px-4 py-2 border border-black rounded-2xl w-[820px] h-full ${lights ? "text-white" : "text-neutral-500"}`}>
                {Array.isArray(keys) && keys.map((data, index) => (
